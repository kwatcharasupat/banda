...
class MockSeparator(nn.Module):
    ...
    def forward(self, audio_chunk: torch.Tensor, **kwargs) -> dict:
        ...
        separated_stems = {
            stem: audio_chunk.clone() for stem in self.expected_output_stems
        }
...
def test_overlap_add_processor_chunking(overlap_add_processor, sample_audio):
    chunked_audio, padded_length, original_length = overlap_add_processor._chunk_audio(
        sample_audio
    )
    ...
    assert (
        chunked_audio.shape[2] == overlap_add_processor.chunk_size_samples
    )  # chunk_size_samples

    # Check if the total length of chunks covers the padded length
    expected_n_chunks = (
        math.ceil(
            (padded_length - overlap_add_processor.chunk_size_samples)
            / overlap_add_processor.hop_size_samples
        )
        + 1
    )
...
def test_overlap_add_processor_process(overlap_add_processor, sample_audio):
    ...
    for stem, audio in separated_audio_device.items():
        assert audio.device == torch.device(
            "cpu"
        )  # Reconstructed audio is moved back to CPU
