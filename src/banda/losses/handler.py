...
from banda.losses.base import LossConfig, LossDict, LossRegistry
...
class LossHandler(_Loss):
    ...
    def forward(self, batch: SourceSeparationBatch) -> LossDict:
        ...
        loss_dict = LossDict(total_loss=total_loss, loss_contrib=loss_contribs)
    ...
    def _build_losses(self):
        ...
        for loss_config in self.config.losses:
            ...
            if name in losses:
                raise ValueError(
                    f"Loss '{name}' is already registered. Use `name` to specify a unique name."
                )
