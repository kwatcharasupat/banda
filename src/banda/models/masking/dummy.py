...
class DummyMaskingModel(_BaseMaskingModel):
    def __init__(self, *, config: DictConfig):
        ...
        self.re_linear = nn.ModuleDict(
            {
                stem: nn.Conv1d(
                    in_channels=config.spectrogram.n_fft // 2 + 1,
                    out_channels=config.spectrogram.n_fft // 2 + 1,
                    kernel_size=(1,),
                )
    ...
    def _inner_model(
        ...
        specs_normalized = torch.reshape(
            specs_normalized, (batch_size * n_channels, n_freq, n_time)
        ).abs()

        masks = {}
        for stem in self.config.stems:
            re_mask = torch.reshape(
                self.re_linear[stem](specs_normalized),
                (batch_size, n_channels, n_freq, n_time),
            )
            im_mask = torch.reshape(
                self.im_linear[stem](specs_normalized),
                (batch_size, n_channels, n_freq, n_time),
            )
