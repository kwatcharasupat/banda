...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels: int, output_features: int) -> None:
        ...
        """
        super().__init__()
        self.conv = nn.Conv1d(input_channels, output_features, kernel_size=16, stride=8)
        self.relu = nn.ReLU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): Input audio tensor. Shape: (batch_size, channels, samples)

        Returns:
            torch.Tensor: Encoded feature tensor. Shape: (batch_size, output_features, encoded_samples)
        """
